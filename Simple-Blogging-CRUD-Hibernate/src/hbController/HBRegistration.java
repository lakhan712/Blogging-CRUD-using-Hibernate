...
import hbmodel.HBRegisterService;
import hbmodel.User;

/**
 * Servlet implementation class Registration
 */
@WebServlet("/HBregister")
public class HBRegistration extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		} else if(request.getAttribute("data").toString() == "1") {
			//System.out.println(request.getAttribute("data").toString());
			response.sendRedirect("hbshow");
			return;
		}
		}catch(Exception e) {
			request.getRequestDispatcher("register.jsp").forward(request, response);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("username").equals("")) {
			...
			HBRegisterService.registerUserHB(user);
